...
class ProcessSyncPaymentMeta extends Job
{
    ...
    protected function save_edd_database()
    {
        ...
        try {
            ...
            $deleted_edd_payments_metas = PaymentMeta::where('store_id', $this->store_id)
            ...
            foreach ($edd_payments_metas as $edd_payment_meta) {
                foreach ($edd_payment_meta as $key => $value) {
                    try {
                        ...
                        PaymentMeta::create($add_arry);
                    } catch(\Exception $e) {
                        \Log::error($e);
                    }
                }
            }
            \Log::info('Successfully Added Payment Meta for ID:'.$this->post_id);
        } catch (\Exception $e) {
            $error = $e->getMessage();
            \Log::error($error);
