...
class ProcessSyncEddLogMeta extends Job
{
    ...
    protected function save_edd_database()
    {
        ...
        try {
            ...
            $deleted_edd_eddlogs_metas = EddLogMeta::where('store_id', $this->store_id)
            ...
            $edd_eddlogs_metas = $response_e_meta->edd_logs_metas;
            foreach ($edd_eddlogs_metas as $edd_eddlog_meta) {
                foreach ($edd_eddlog_meta as $key => $value) {
                    EddLogMeta::create([
            ...
            \Log::info('Successfully Added edd log Meta for ID:'.$this->post_id);
        } catch (\Exception $e) {
            $error = $e->getMessage();
            \Log::error($error);
